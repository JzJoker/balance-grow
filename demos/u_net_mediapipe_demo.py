...
HAIR_CLASS = 1  # adjust if your dataset uses different label
...
base_options=BaseOptions(model_asset_path="hair_segmenter.tflite"),    running_mode=RunningMode.IMAGE,
    output_category_mask=True
)
mp_segmenter = ImageSegmenter.create_from_options(mp_options)

def predict_mp_mask(frame):
    mp_frame = mpImage(image_format=mpImage.ImageFormat.SRGB, data=cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
